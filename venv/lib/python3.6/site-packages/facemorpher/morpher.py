...
def morph_one(src_img, src_points, dest_img, dest_points,
              percent, width=500, height=600):
  size = (height, width)
  points = locator.weighted_average_points(src_points, dest_points, percent)
  src_face = warper.warp_image(src_img, src_points, points, size)
  end_face = warper.warp_image(dest_img, dest_points, points, size)
...
def morph_ani(src_imgpaths, des_imgpath, num_pics, width=500, height=600, fps_in=24, slow_rate = 5,
  ...
  for src_img, src_points in images_points_gen:
    ...
    avg_face0, avg_points0 = morph_one(src_img, src_points, dest_img, dest_points,
                             1 - float(frameCount) / (num_frames - 1), width, height)
    for i in range(0,slow_rate):
        avg_face, avg_points = morph_one(avg_face0, avg_points0, p_face, p_points,
                                         i / float(slow_rate - 1), width, height)
